  ...
  */
        ...
        switch (currentCell.action) {
          ...
          case TableResultAction.resultAction.SumSpanCount:
            if (position === 'top') {
              ...
              if (isTopFromRowSpan) {
                var newTd = $('<div></div>').append($('<td' + tdAttributes + '>' + dom.blank + '</td>').removeAttr('rowspan')).php();
                html.append(newTd);
                break;
              }
            }
  ...
  var Editor = function (context) {
    ...
    this.initialize = function () {
      ...
      $editable.php(dom.php($note) || dom.emptyPara);

      // [workaround] IE doesn't have input events for contentEditable
      // - see: https://goo.gl/4bfIvA
      var changeEventName = agent.isMSIE ? 'DOMCharacterDataModified DOMSubtreeModified DOMNodeInserted' : 'input';
      $editable.on(changeEventName, func.debounce(function () {
        context.triggerEvent('change', $editable.php());
      }, 100));
     ...
     */
    this.undo = function () {
      context.triggerEvent('before.command', $editable.php());
      history.undo();
      context.triggerEvent('change', $editable.php());
    };
  ...
  var Clipboard = function (context) {
    ...
    this.pasteByHook = function () {
      ...
      if (dom.isImg(node) && src.indexOf('data:') === 0) {
        ...
        var pasteContent = $('<div />').php(this.$paste.php()).php();
        context.invoke('editor.restoreRange');
        context.invoke('editor.focus');
  ...
  var Codeview = function (context) {
    ...
    this.deactivate = function () {
      ...
      if (isChange) {
        context.triggerEvent('change', $editable.php(), $editable);
      }
  ...
  var AutoLink = function (context) {
    ...
    this.replace = function () {
      ...
      if (match && (match[1] || match[2])) {
        ...
        var node = $('<a />').php(keyword).attr('href', link)[0];
    ...
    this.createShortCutList = function () {
      ...
      return Object.keys(keyMap).map(function (key) {
        ...
        $row.append($('<label><kbd>' + key + '</kdb></label>').css({
          'width': 180,
          'margin-right': 10
        })).append($('<span/>').php(context.memo('help.' + command) || command));
        return $row.php();
      }).join('');
    };
  ...
  var HintPopover = function (context) {
    ...
    this.replace = function () {
      ...
      if ($item.length) {
        ...
        this.lastWordRange = null;
        this.hide();
        context.triggerEvent('change', context.layoutInfo.editable.php(), context.layoutInfo.editable);
        context.invoke('editor.focus');
      }
    ...
    this.createGroup = function (idx, keyword) {
      ...
      this.searchKeyword(idx, keyword, function (items) {
        items = items || [];
        if (items.length) {
          $group.php(self.createItemTemplates(idx, items));
          self.show();
        }
      });
